...
async function api(path, opt = {}) {
  ...
  try {
    ...
    if (/failed to fetch|network|internet|load failed|delai|timeout|reveil/i.test(m)) {
      throw new Error('Connexion impossible. Verifiez le reseau ou attendez le reveil du serveur (30-60s).');
    }
    throw e;
  }
...
function updateAutoUI(on, data) {
  ...
  if ($('autoMsg')) {
    ...
    $('autoMsg').textContent = on
      ? 'Mise automatique activee.'
      : (data && data.error) || 'Mise automatique desactivee.';
  }
}
...
function toggleAutopilot() {
  ...
  if ($('autopilotMsg')) {
    ...
    $('autopilotMsg').textContent = on ? 'Autopilote demande.' : 'Autopilote desactive.';
  }
}
...
async function loadPronos() {
  ...
  try {
    ...
    if (!rows.length) {
      box.innerHTML = '<div class="empty">Aucune donnee radar pour le moment.</div>';
      return;
    }
    box.innerHTML = '<div class="notice info" style="margin-bottom:10px">' + rows.length + ' match(s)</div>' + rows.slice(0, 60).map(matchCard).join('');
  } catch (e) {
    ...
    if (!rows.length) {
      try {
        ...
        rows = [].concat(d.data || []).filter(x => {
          ...
          const id = Number(x.statusId);
          return id > 0 && id < 90 && !/pre-game|not started|scheduled|finished|ended|cancelled/i.test(s);
        });
      } catch (e2) { throw e2; }
    }
    box.innerHTML = rows.length
      ? rows.slice(0, 50).map(matchCard).join('')
      : '<div class="empty">Aucun match en cours. Voir Radar / Pre-match.</div>';
  } catch (e) { box.innerHTML = `<div class="notice bad">${esc(e.message)}</div>`; }
}
  ...
  try {
    ...
    if (!rows.length) {
      ...
      rows = [].concat(d.data || []).filter(x => {
        const s = String(x.statusName || x.status || '').toLowerCase();
        const id = Number(x.statusId);
        return id === 0 || /pre|not started|scheduled|upcoming/i.test(s);
      });
    }
    box.innerHTML = rows.length
      ? rows.slice(0, 50).map(matchCard).join('')
...
async function loadOpps() {
  ...
  try {
    ...
    box.innerHTML = n
      ? `<div class="notice info">${n} fixture(s) radar. Aucune surebet inventee.</div>`
      : '<div class="empty">Aucune opportunite pour le moment.</div>';
  } catch (e) {
    box.innerHTML = `<div class="notice bad">${esc(e.message)}</div>`;
  }
}
