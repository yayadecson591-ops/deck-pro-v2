...
function withTimeout(promise, ms, label) {
  return Promise.race([
    promise,
    new Promise((_, rej) => setTimeout(() => rej(new Error(label || 'Delai depasse')), ms))
  ]);
}
async function api(path, opt = {}) {
  ...
  try {
    ...
    }), 45000, 'Serveur indisponible ou en reveil');
  } catch (e) {
    const m = String(e && e.message || e);
    if (/failed to fetch|network|internet|load failed|delai|timeout|reveil/i.test(m)) {
      throw new Error('Connexion impossible. Verifiez le reseau ou attendez le reveil du serveur (30-60 s).');
    }
    throw e;
  }
...
const TAB_LABELS = {
  home: 'TABLEAU DE BORD', books: 'MES BOOKMAKERS', opps: 'OPPORTUNITES', radar: 'RADAR',
  live: 'LIVE', pre: 'PRE-MATCH', autobet: 'MISE AUTOMATIQUE', journal: 'JOURNAL', settings: 'PARAMETRES'
};
...
async function health() {
  if ($('pill')) { $('pill').textContent = 'REVEIL…'; $('pill').className = 'pill off'; }
  if ($('sideStatus')) $('sideStatus').textContent = 'Reveil du serveur…';
  try {
    const d = await withTimeout(fetch(API + '/health').then(r => r.json()), 90000, 'Serveur en reveil').catch(() => null);
    ...
    if (operational || (d && d.ok)) {
      if ($('sideStatus')) $('sideStatus').textContent = operational ? 'Systeme actif' : 'Serveur en ligne';
      if ($('sideDot')) $('sideDot').className = 'status-dot';
      if ($('pill')) { $('pill').textContent = operational ? 'SYSTEME ACTIF' : 'EN LIGNE'; $('pill').className = 'pill'; }
    ...
    if ($('sideStatus')) $('sideStatus').textContent = 'Serveur en reveil — patientez 30–60s';
...
function statusForSlug(slug) {
  ...
  if (!conns.length) return { key: 'off', label: 'DECONNECTE', cls: 'off', card: 'disconnected' };
  const c = conns[0];
  const st = String(c.status || '').toLowerCase();
  if (st === 'expired' || st === 'invalid' || st === 'revoked') return { key: 'warn', label: 'ACCES EXPIRE', cls: 'warn', card: 'expired', conn: c };
  if (st === 'syncing' || st === 'pending') return { key: 'sync', label: 'SYNCHRONISATION', cls: 'sync', card: 'connected', conn: c };
  return { key: 'ok', label: 'CONNECTE', cls: 'ok', card: 'connected', conn: c };
}
...
function bookCard(slug) {
  ...
  const balance = conn && (conn.balance != null || conn.solde != null) ? fmtMoney(conn.balance ?? conn.solde) : 'Non synchronise';
  ...
  return `<div class="book-card ${st.card}">
    ...
    <div class="book-meta">
      ...
      <div>Derniere sync : <b>${esc(lastSync)}</b></div>
...
async function loadBooks() {
  ...
  let banner = '';
  if (connErr || !tok()) {
    banner = `<div class="notice warn" style="grid-column:1/-1;margin-bottom:8px">Session requise. Parametres → code proprietaire → Ouvrir session.</div>`;
  }
...
async function syncBookmaker() {
  ...
  try {
    ...
    if (msg) { msg.className = 'notice good'; msg.textContent = d.message || 'Connexion enregistree.'; }
    await loadBooks();
    setTimeout(closeConnectModal, 900);
  } catch (e) { if (msg) { msg.className = 'notice bad'; msg.textContent = e.message || 'Echec'; } }
}
...
function updateAutoUI(on, data) {
  ...
  if ($('autoToggleBtn')) {
    $('autoToggleBtn').textContent = on ? 'Desactiver' : 'Activer';
  ...
  if ($('statAutoSub')) $('statAutoSub').textContent = on ? 'activee' : 'desactivee';
  if ($('autoChecks')) {
    ...
    $('autoChecks').innerHTML = `<div class="${hasConn ? 'ok' : 'fail'}">Connexions persistantes${hasConn ? '' : ' — aucun compte'}</div>
      <div class="pend">Canal d'execution</div><div class="pend">Revalidation cotes</div>
  ...
  if ($('autoMsg')) {
    ...
    $('autoMsg').textContent = on
      ? 'Mise automatique activee (placements via canaux reels apres verifications).'
      : (data && data.error) || 'Mise automatique desactivee. Connectez un bookmaker puis activez.';
  }
}
async function toggleGlobalAuto() {
  const currentlyOn = $('autoLabel') && $('autoLabel').textContent === 'ON';
  try {
    await api('/api/account/auto-stake', { method: 'POST', body: JSON.stringify({ enabled: !currentlyOn, confirmationId: 'ui-' + Date.now() }) });
...
function toggleAutopilot() {
  ...
  if ($('autopilotMsg')) {
    ...
    $('autopilotMsg').textContent = on ? 'Autopilote demande (cote serveur).' : 'Autopilote desactive.';
  }
}
async function loadHome() {
  ...
  if (preview) {
    if (!tok()) preview.innerHTML = '<div class="empty">Session fermee. <button class="btn sm primary" onclick="tab(\'settings\')">Ouvrir session</button></div>';
...
async function loadHistoryPreview() {
  ...
  try {
    ...
    if (!rows.length) { box.innerHTML = '<div class="empty">Aucune operation.</div>'; return; }
    box.innerHTML = rows.slice(0, 5).map(h => {
      const title = h.title || h.event || h.type || 'Operation';
...
async function loadHistory() {
  ...
  try {
    const d = await api('/api/account/history');
    const rows = d.history || d.rows || [];
    if (!rows.length) { box.innerHTML = '<div class="empty">Aucune operation enregistree.</div>'; return; }
...
async function loadPronos() {
  ...
  try {
    ...
    box.innerHTML = rows.length ? rows.slice(0, 40).map(matchCard).join('') : '<div class="empty">Aucune donnee pour le moment.</div>';
  } catch (e) {
  ...
  try {
    ...
    try {
      ...
      rows = [].concat(d.data || []).filter(x => {
        ...
        return /live|in play|in-play|1st|2nd|half|period|set|quarter/i.test(s);
      });
    }
    box.innerHTML = rows.length
      ? rows.slice(0, 40).map(matchCard).join('')
      : '<div class="empty">Aucun match live pour le moment.</div>';
  } catch (e) { box.innerHTML = `<div class="notice bad">${esc(e.message)}</div>`; }
}
async function loadPre() {
  const box = $('preList');
  if (!box) return;
  box.innerHTML = '<div class="empty">Chargement pre-match…</div>';
  try {
    let rows = [];
    try {
    ...
    box.innerHTML = rows.length
      ? rows.slice(0, 40).map(matchCard).join('')
      : '<div class="empty">Aucun pre-match pour le moment.</div>';
  } catch (e) { box.innerHTML = `<div class="notice bad">${esc(e.message)}</div>`; }
}
async function loadOpps() {
  ...
  try {
    ...
    box.innerHTML = n
      ? `<div class="notice info">${n} fixture(s) radar. Aucune surebet inventee — uniquement des calculs reels.</div>`
      : '<div class="empty">Aucune opportunite reelle pour le moment.</div>';
  } catch (e) {
    box.innerHTML = `<div class="notice bad">${esc(e.message)}</div>`;
  }
}
...
async function softActivate() {
  try {
    ...
    if ($('lockMsg')) { $('lockMsg').className = 'notice bad'; $('lockMsg').textContent = e.message || 'Echec'; }
  }
}
async function boot() {
  await health();
