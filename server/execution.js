...
// A route is verified either by the explicit strategy registry or by a provider
// adapter whose complete production contract is configured and authorized.
// This removes the previous deadlock where a ready provider adapter could never
// pass preflight because execution-strategies.js had no hard-coded verified flag.
function verifiedRoute(slug, channel){
  if(isExecutionChannelVerified(slug,channel)) return true;
  if(!['partner_sdk','official_api','authorized_gateway'].includes(channel)) return false;
  const provider=providerExecutionAdapterStatus().find(x=>x.bookmaker===String(slug||'').trim().toLowerCase());
  return Boolean(provider?.ready);
}
...
export function executionCapabilities(){return Object.values(adapters).map(a=>{const verified=supportedChannels().filter(channel=>verifiedRoute(a.slug,channel));const provider=providerForBookmaker(a.slug);const providerAdapter=providerExecutionAdapterStatus().find(x=>x.bookmaker===a.slug);const onboarding=bookmakerOnboardingFor(a.slug);const adapter=executionAdapterStatus(a.slug);return{bookmaker:a.slug,name:a.name,enabled:enabled(a.slug),configuredChannels:a.channels.filter(channel=>Boolean(env(`${a.slug.toUpperCase().replace(/[^A-Z0-9]/g,'_')}_${channel.toUpperCase()}_URL`))),userTokenConfigured:tokenEnabled(a.slug),strategy:executionStrategy(a.slug),verifiedFallbackChannels:verified,provider:provider?{id:provider[0],name:provider[1].name,scope:provider[1].scope,status:provider[1].status}:null,providerAdapter:providerAdapter||null,onboarding:onboarding?{route:onboarding.route,status:onboarding.status,provider:onboarding.provider||null,contact:onboarding.contact||null,configured:onboarding.credentials.every(key=>Boolean(env(key)))}:null,adapter,executable:Boolean(enabled(a.slug)&&verified.length&&adapter?.authorized&&adapter?.documented),status:verified.length?'VERIFIED_ROUTE_AVAILABLE':'TECHNICAL_ACCESS_RESEARCH'}})}
