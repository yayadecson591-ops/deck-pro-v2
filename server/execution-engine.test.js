...
import { executeTwoLegTransaction, getExecutionTransaction, reconcileExecutionTransaction, executionEngineStatus, validateAutoExecutionWindow } from './execution-engine.js';
...
test('two-leg engine exposes decision gate',()=>{const status=executionEngineStatus();assert.equal(status.exactlyTwoBookmakers,true);assert.equal(status.reconciliationStateMachine,true);assert.equal(status.autoStakeGate,true);assert.equal(status.decisionGate,true);assert.equal(status.preMatchAutoWindowMaxDays,7)});
...
test('pre-match auto execution policy rejects more than seven days',()=>{const result=validateAutoExecutionWindow({area:'pre-match',autoWindowDays:8,startTime:new Date(Date.now()+8*86400000).toISOString()},true);assert.equal(result.ok,false);assert.equal(result.code,'PREMATCH_AUTO_WINDOW_INVALID')});
test('pre-match auto execution policy rejects a match outside selected window',()=>{const result=validateAutoExecutionWindow({area:'pre-match',autoWindowDays:3,startTime:new Date(Date.now()+5*86400000).toISOString()},true);assert.equal(result.ok,false);assert.equal(result.code,'PREMATCH_AUTO_HORIZON_EXCEEDED')});
test('pre-match auto execution policy accepts a match inside selected window',()=>{const result=validateAutoExecutionWindow({area:'pre-match',autoWindowDays:7,startTime:new Date(Date.now()+2*86400000).toISOString()},true);assert.equal(result.ok,true);assert.equal(result.days,7)});
