...
async function oddsPapi(pathname,params={}){if(!API_KEY)throw Object.assign(new Error('Source live OddsPapi non configurée (ODDSPAPI_API_KEY).'),{status:503,code:'ODDSPAPI_NOT_CONFIGURED'});const q=new URLSearchParams({apiKey:API_KEY,language:'en',...params});const r=await fetch(`${ODDS_BASE}${pathname}?${q}`,{headers:{Accept:'application/json'},signal:AbortSignal.timeout(15000)});const text=await r.text();let data;try{data=JSON.parse(text)}catch{data={error:text.slice(0,300)}}if(!r.ok){const msg=typeof data?.message==='string'?data.message:(typeof data?.error==='string'?data.error:(data?.error?.message||`OddsPapi HTTP ${r.status}`));throw Object.assign(new Error(String(msg)),{status:r.status,code:'ODDSPAPI_ERROR'});}return data}
