...
app.get('/health',(req,res)=>res.json({ok:true,service:'deck-pro-server',version:'V122',status:'online',uptimeSec:Math.floor(process.uptime())}));
...
app.get('/api/system/config-check',auth,(req,res)=>res.json({ok:true,checks:{oddsPapi:!!API_KEY,ownerAuth:!!effectiveHash&&!!effectiveSecret,origins:ORIGINS.length>0,bookmakers:BOOKMAKERS.length>0},version:'V122'}));
...
app.listen(PORT,'0.0.0.0',()=>console.log(`Deck Pro V122 server listening on :${PORT}`));
