...
function comparableKey(leg){return [String(leg.eventId||leg.fixtureId||leg.matchId||''),String(leg.marketId||leg.market||''),String(leg.line??''),String(leg.period||'full'),String(leg.outcome||leg.selection||'')].join('|')}
...
function buildArbitrage(legs,total,step=0.01){
  ...
  if(Math.abs(sum-total)>1e-9){
    ...
    const idx=stakes.reduce((best,_,i)=>raw[i]>raw[best]?i:best,0);
    stakes[idx]=Math.max(0,Math.round((stakes[idx]+delta)/step)*step);
  }
...
app.get('/health',(req,res)=>res.json({ok:true,service:'deck-pro-server',version:'V123',status:'online',uptimeSec:Math.floor(process.uptime())}));
...
app.get('/api/system/config-check',auth,(req,res)=>res.json({ok:true,checks:{oddsPapi:!!API_KEY,ownerAuth:!!effectiveHash&&!!effectiveSecret,origins:ORIGINS.length>0,bookmakers:BOOKMAKERS.length>0},version:'V123'}));
